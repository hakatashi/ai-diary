...
- **実機接続で判明した重要な制約**: `.psm1`のモジュールスコープ(トップレベルで`$script:foo = ...`のように定義した変数)は、Playniteが`OnGameStopped`等のイベント関数を実際に呼び出す際には参照できず`$null`になる(`Import-Module`後に手動で関数を呼ぶ通常のPowerShellセッションでは問題なく動作するため、開発中は気づきにくい)。null値を`Test-Path`/`Add-Content`等の`-Path`引数に渡すと例外になり、それが`try`の外側だと呼び出しごと失敗し、内側だと`catch`で握りつぶされて症状が全く見えなくなる(`Write-Host`の出力もPlaynite上のどこにも表示・記録されないため尚更気づけない)。対策として、設定ファイルパス等はイベント関数の呼び出しのたびに関数内でローカル変数として計算すること。また`$env:APPDATA`も同様に信頼できない場合があるため、`[Environment]::GetFolderPath([Environment+SpecialFolder]::ApplicationData)`で取得すること。合わせて`%APPDATA%\AiDiaryPlaynite\debug.log`へのファイルログ出力を追加し、`Write-Host`が実質観測不能である問題を回避した。
