...
- **`dataPoints.list` のフィルタ構文はデータタイプの「種類」によって異なる**(公式リファレンス https://developers.google.com/health/reference/rest/v4/users.dataTypes.dataPoints/list で確認):
  - `nutrition-log` は `exercise` と同じセッション種別のため、`nutrition-log.interval.civil_start_time >= "..." AND ... < "..."` という同じcivil date範囲フィルタパターンを使う(`client.ts` の `buildCivilDateRangeFilter` を共通化して両方から利用)。
  ...
  - sleep/weightのタイムスタンプフィルタは `buildTimestampRangeFilter` として共通化(`client.ts`)。両者ともコンパレータは `>=`/`<` のみサポート(exerciseのcivil_start_timeと同様の制約)という前提で実装しているが、**sleep/nutrition-log/weightは実接続で未検証**(このMCP経由の実データ取得はai-diaryのOAuthクライアントを経由していないため、実際のスコープ・フィルタ挙動は初回のGoogle Health再接続・同期時にトライアル&エラーの修正が必要になる可能性がある。Swarm連携と同様のリスク)。
