...
- **各データタイプは運動記録とは別のOAuthスコープが必要**(公式リファレンス https://developers.google.com/health/scopes 参照)。食事は `googlehealth.nutrition.readonly`、睡眠は `googlehealth.sleep.readonly`、体重は `googlehealth.health_metrics_and_measurements.readonly`。既存にGoogle Healthを接続済みのユーザーは、GPSスコープ追加時と同様に**再接続(OAuth再同意)が必要**(下記「手動セットアップチェックリスト」参照)。未同意の状態でその種別のAPIを呼ぶと403 `MISSING_OAUTH_SCOPE` になることを実接続(Google Health MCPツール経由、`health_metrics_and_measurements.readonly`未同意で体重取得が403になるケース)で確認済み。
- **`dataPoints.list` のフィルタ構文はデータタイプの「種類」によって異なる**(公式リファレンス https://developers.google.com/health/reference/rest/v4/users.dataTypes.dataPoints/list で確認):
  - `nutrition-log` は `exercise` と同じセッション種別のため、civil date範囲フィルタパターンを使う。**ただしfilter式のパス先頭はURLパスセグメント(kebab-case)の `nutrition-log` ではなく、JSONレスポンスのフィールド名(camelCase)である `nutritionLog` を使う必要がある**(`nutritionLog.interval.civil_start_time >= "..." AND ... < "..."`)。実接続で `nutrition-log.interval.civil_start_time` を使ったところ `INVALID_DATA_POINT_FILTER_DATA_TYPE_RESTRICTION`(`Restriction member path segment 'nutrition-log' does not match any data type`)エラーとなり判明・修正した。exercise/sleep/weightはdataTypeIdとJSONフィールド名が一致する(ハイフンを含まない)ため、この問題は起きない(`client.ts` の `buildCivilDateRangeFilter` を共通化して利用)。
  - `sleep` はセッション種別の例外(ECGと同様)で、civil dateではなく素の `sleep.interval.end_time >= "ISO" AND ... < "ISO"` というRFC 3339タイムスタンプによる範囲フィルタが使える(公式リファレンスに明記)。
  - `weight` はサンプル種別のデータタイプで、`weight.sample_time.physical_time >= "ISO" AND ... < "ISO"` というタイムスタンプ範囲フィルタを使う。
  - sleep/weightのタイムスタンプフィルタは `buildTimestampRangeFilter` として共通化(`client.ts`)。両者ともコンパレータは `>=`/`<` のみサポート(exerciseのcivil_start_timeと同様の制約)という前提で実装しており、**sleep/weightは実接続で未検証**(nutrition-logはフィルタのパス先頭の問題を実接続で修正済みだが、sleep/weightは同様の問題が潜在している可能性がある。Swarm連携と同様のトライアル&エラーのリスク)。
- **レスポンスのJSONスキーマ**(公式リファレンスに基づく、int64型フィールドはprotobufのJSON表現として文字列で返る点に注意):
  - `nutrition-log`: `{name, nutritionLog: {interval: {startTime, endTime}, mealType, foodDisplayName, energy: {value}}}`。`mealType`(`BREAKFAST`/`LUNCH`/`DINNER`/`SNACK`等)を日本語ラベル化してtitleに、`foodDisplayName` をsummaryに、`energy.value`(kcal、実接続のFitbit由来データでは常にkcal単位と想定)を `metrics.calories` に格納する(`normalize.ts` の `normalizeNutritionLog`)。栄養素の詳細(`nutrients[]`、`totalCarbohydrate`/`totalFat`等)は現時点で未活用(rawには保持)。
  - `sleep`: `{name, sleep: {interval: {startTime, endTime}, summary: {minutesAsleep, stagesSummary: [{type, minutes, count}]}}}`。`minutesAsleep` を `metrics.durationMinutes` に、`stagesSummary` を日本語ラベル化して睡眠段階の内訳文字列としてsummaryに整形する(`normalize.ts` の `normalizeSleep`/`formatSleepSummary`)。
  - `weight`: `{name, weight: {sampleTime: {physicalTime}, notes, weightGrams}}`。`weightGrams / 1000` を新設した `metrics.weightKilograms` に格納する(`normalize.ts` の `normalizeWeight`)。
...
18. デプロイ・再接続後、`/data-sources` から「今すぐ同期」を実行し、食事・睡眠・体重の記録が日誌ページに表示されることを確認する。`sleep`/`nutrition-log`/`weight` の `dataPoints.list` フィルタ構文は公式リファレンスに基づく実装で実接続未検証のため、エラーが出る場合はフィルタ構文(`functions/src/dataSources/googleHealth/client.ts`)の見直しが必要になる可能性がある。
